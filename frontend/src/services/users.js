...
export const updatePrivacy = (payload) => unwrap(api.put('/users/privacy', payload));
export const getUser = (id) => unwrap(api.get(`/users/${id}`));
export const getPortfolio = (id) => unwrap(api.get(`/users/${id}/portfolio`));
...
export const uploadCoverPhoto = (file) => {
  ...
  return unwrap(api.post('/users/cover-photo', form, { headers: { 'Content-Type': 'multipart/form-data' } }));
};
...
export const endorseSkill = (payload) => unwrap(api.post('/users/endorse', payload));
export const getEndorsements = (userId) => unwrap(api.get(`/users/${userId}/endorsements`));
